...
async function kakaoKeywordSearch({ query, rect, x, y, page }) {
    ...
    if (rect)
        params.rect = rect; // swLng,swLat,neLng,neLat
    else if (x && y) {
        params.x = x;
        params.y = y;
        params.radius = 20000; // 20km 기본 반경
    }
...
app.get('/api/search', async (req, res) => {
    try {
        ...
        const useRect = hasBounds && deltaLat <= 0.5 && deltaLng <= 0.5; // rect 영역이 너무 크면 카카오 API가 빈 응답을 반환할 수 있어 제한
